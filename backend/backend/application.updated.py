...
if MODEL_PATH is None:
    ...
    if DEV_MODE:
        ...
        print(" PRODUCTION MODE: Continuing without model. /api/predict will return 503 until a model is available.\n")
...
def ensure_model_loaded():
    """Load the model on-demand (avoids slow startup/healthcheck failures)."""
    global model, infer, MODEL_LOAD_ERROR

    if model is not None and infer is not None:
        return True, None

    if not MODEL_PATH:
        return False, "MODEL_PATH is empty (model file not present on the server)."
    ...
    with MODEL_LOAD_LOCK:
        ...
        try:
            ...
            model_local = keras.models.load_model(
                MODEL_PATH,
            ...
            def _infer(x):
                return model_local(x, training=False)

            infer_local = _infer
            _ = infer_local(tf.zeros((1, 300, 300, 3), dtype=tf.float32))
